...
function initStartScreen() {
    ...
    userInput.addHitArea("showGarage", butEventHandler, null, "rect", { aRect: [310, 310, 420, 390] }, !0);
    var c = new Array(a, b);
...
function initMapScreen() {
    ...
    currentCarId = saveDataHandler.aLevelStore[32],
...
function initGame() {
    ...
    userCar = new Elements.UserCar(assetLib.getData("userCar" + currentCarId), canvas.width, canvas.height),
    enemySpeed = 390 + 7.2 * levelNum,
    raceLength = 4e4 + 1e3 * levelNum,
    maxSpeed = aCarData[currentCarId].maxSpeed + 6.75 * aPowerUpBarData[1],
    accRate = aCarData[currentCarId].accRate - .32 * aPowerUpBarData[2],
    turnRate = aCarData[currentCarId].turnRate + .375 * aPowerUpBarData[0],
...
function butEventHandler(a, b) {
    ...
    case "showGarage":
        ...
        userInput.removeHitArea("showMapScreen"),
        userInput.removeHitArea("credits"),
        userInput.removeHitArea("showGarage"),
        initGarage();
        break;
    case "backFromGarage":
        playSound("click"),
        userInput.removeHitArea("backFromGarage"),
        userInput.removeHitArea("prevCarGarage"),
        userInput.removeHitArea("nextCarGarage"),
        userInput.removeHitArea("buyCarGarage"),
        userInput.removeHitArea("selectCarGarage"),
        initStartScreen();
        break;
    case "prevCarGarage":
        garageCarIdx = (garageCarIdx - 1 + aCarData.length) % aCarData.length;
        break;
    case "nextCarGarage":
        garageCarIdx = (garageCarIdx + 1) % aCarData.length;
        break;
    case "buyCarGarage":
        var gCar = aCarData[garageCarIdx];
        if (winnings >= gCar.price && !(saveDataHandler.aLevelStore[33] & (1 << garageCarIdx))) {
            playSound("upgrade"),
            winnings -= gCar.price,
            saveDataHandler.aLevelStore[33] |= (1 << garageCarIdx),
            saveDataHandler.aLevelStore[31] = winnings,
            saveDataHandler.saveData()
        }
        break;
    case "selectCarGarage":
        if (saveDataHandler.aLevelStore[33] & (1 << garageCarIdx)) {
            playSound("click"),
            currentCarId = garageCarIdx,
            saveDataHandler.aLevelStore[32] = currentCarId,
            saveDataHandler.saveData()
        }
        break;
    case "moreGames":
...
function initLevelComplete() {
    ...
    var posBonus = 0 == racePos ? 200 : 1 == racePos ? 100 : 2 == racePos ? 50 : 0;
    var crashPenalty = crashCount * 10;
    var raceCoinsEarned = coinsThisRace + posBonus;
    winnings += raceCoinsEarned,
    saveDataHandler.aLevelStore[31] = winnings,
    panel.oScoreData = {
        racePos: racePos,
        winnings: raceCoinsEarned,
        ...
        totalScore: totalScore,
        coinsThisRace: coinsThisRace,
        posBonus: posBonus,
        crashPenalty: crashPenalty,
        overtakeCoins: overtakeCoins,
        totalCoins: winnings
    },
...
function roadCallback(a, b) {
    ...
    case "hitEnemyCar":
        ...
        4 >= startTimer && (levelScore -= 100, crashCount++, coinsThisRace = Math.max(0, coinsThisRace - 10)),
        crashFlashTimer = 0.3,
        addFloatText("-10", canvas.width / 2, 300, "#ff4444"),
    ...
    case "hitCoin":
        coinsThisRace += 5,
        addFloatText("+5", road.aRowData ? canvas.width / 2 : canvas.width / 2, 280, "#f0c030");
        break;
    }
}
...
function renderThemeOverlay(ctx) {
    var themeGrad = {
        forest: ["rgba(30,80,30,0.25)", "rgba(0,0,0,0)"],
        city:   ["rgba(0,30,80,0.35)", "rgba(80,0,120,0.2)"],
        desert: ["rgba(100,60,0,0.25)", "rgba(0,0,0,0)"]
    };
    var cols = themeGrad[levelTheme] || ["rgba(0,0,0,0.1)", "rgba(0,0,0,0)"];
    var grad = ctx.createLinearGradient(0, 0, canvas.width, canvas.height);
    grad.addColorStop(0, cols[0]);
    grad.addColorStop(1, cols[1]);
    ctx.fillStyle = grad;
    ctx.fillRect(0, 0, canvas.width, canvas.height);
    // City theme: neon border
    if (levelTheme === "city") {
...
function initGarage() {
    gameState = "garage",
    winnings = saveDataHandler.aLevelStore[31],
    garageCarIdx = saveDataHandler.aLevelStore[32],
    background = new Elements.Background(assetLib.getData("mainBackground"), canvas.width, canvas.height),
    userInput.addHitArea("backFromGarage", butEventHandler, null, "rect", { aRect: [20, 355, 160, 395] }, !0),
    userInput.addHitArea("prevCarGarage", butEventHandler, null, "rect", { aRect: [40, 120, 120, 280] }, !0),
    userInput.addHitArea("nextCarGarage", butEventHandler, null, "rect", { aRect: [580, 120, 660, 280] }, !0),
    userInput.addHitArea("buyCarGarage", butEventHandler, null, "rect", { aRect: [200, 310, 370, 355] }, !0),
    userInput.addHitArea("selectCarGarage", butEventHandler, null, "rect", { aRect: [370, 310, 540, 355] }, !0),
    previousTime = (new Date).getTime(),
    updateGarageEvent()
}
function renderGarageScreen() {
    ...
    // Title bar
    var grad = ctx.createLinearGradient(0, 0, W, 50);
    ...
    ctx.textAlign = "left";
    ctx.fillText("CAR GARAGE", 20, 33);
    // Coins display
    ctx.fillStyle = "#f0c030";
    ctx.font = "bold 18px Arial";
    ctx.textAlign = "right";
    ctx.fillText("\u2665 " + winnings + " coins", W - 20, 33);
    // Car preview panel
    ctx.fillStyle = "rgba(255,255,255,0.05)";
    var r = function(x,y,w,h,rad){ ctx.beginPath(); if(ctx.roundRect){ctx.roundRect(x,y,w,h,rad);}else{ctx.rect(x,y,w,h);} ctx.fill(); };
    r(130, 90, 440, 200, 12);
    ctx.strokeStyle = "rgba(240,192,48,0.4)";
    ctx.lineWidth = 1;
    ctx.stroke();
    // Draw selected car
    var carData = assetLib.getData("userCar" + garageCarIdx);
    if (carData) {
        var sw = carData.oData.spriteWidth, sh = carData.oData.spriteHeight;
        var scale = Math.min(300 / sw, 120 / sh);
        var dw = sw * scale, dh = sh * scale;
        // Draw car body (frame 4 = straight)
        ctx.drawImage(carData.img, 3 * sw, 0, sw, sh, W/2 - dw/2, 200 - dh/2 + 30, dw, dh);
        ctx.drawImage(carData.img, 4 * sw, 0, sw, sh, W/2 - dw/2, 200 - dh/2 + 20, dw, dh);
    }
    // Car name
    ctx.fillStyle = "#ffffff";
    ctx.font = "bold 20px Arial";
    ctx.textAlign = "center";
    ctx.fillText(aCarData[garageCarIdx].name, W/2, 110);
    // Stat bars
    var stats = [
        { label: "Top Speed", val: aCarData[garageCarIdx].maxSpeed, min: 440, max: 560 },
        { label: "Handling",  val: aCarData[garageCarIdx].turnRate, min: 1.4, max: 2.6 },
        { label: "Accel",     val: 6 - aCarData[garageCarIdx].accRate, min: 1.0, max: 3.0 }
    ];
    var barY = 305, barW = 200, barH = 8;
    ctx.font = "12px Arial";
    stats.forEach(function(s, i) {
        var bx = 160 + i * 175, by = barY;
        var pct = (s.val - s.min) / (s.max - s.min);
        ctx.fillStyle = "rgba(255,255,255,0.15)";
        ctx.fillRect(bx, by, barW * 0.7, barH);
        var barColor = i === 0 ? "#f0c030" : i === 1 ? "#30d0f0" : "#f07030";
        ctx.fillStyle = barColor;
        ctx.fillRect(bx, by, barW * 0.7 * pct, barH);
        ctx.fillStyle = "rgba(255,255,255,0.7)";
        ctx.textAlign = "left";
        ctx.fillText(s.label, bx, by - 4);
    });
    // Arrow buttons
    var drawArrow = function(x, cx, cy, dir) {
        ctx.fillStyle = "rgba(255,255,255,0.15)";
        ctx.beginPath();
        if(ctx.roundRect){ctx.roundRect(x, 140, 60, 100, 8);}else{ctx.rect(x, 140, 60, 100);}
        ctx.fill();
        ctx.fillStyle = "#ffffff";
    ...
    drawArrow(40, 70, 196, "<");
    drawArrow(600, 630, 196, ">");
    // Owned / Buy / Select buttons
    var owned = (saveDataHandler.aLevelStore[33] & (1 << garageCarIdx)) !== 0;
    var isActive = saveDataHandler.aLevelStore[32] === garageCarIdx;
    // Buy button
    if (!owned) {
        var canAfford = winnings >= aCarData[garageCarIdx].price;
        ctx.fillStyle = canAfford ? "#f0c030" : "rgba(150,150,150,0.5)";
        ctx.beginPath();
        if(ctx.roundRect){ctx.roundRect(200, 330, 160, 36, 8);}else{ctx.rect(200, 330, 160, 36);}
        ctx.fill();
        ctx.fillStyle = canAfford ? "#1a1a1a" : "#888";
        ctx.font = "bold 14px Arial";
        ctx.textAlign = "center";
        ctx.fillText("BUY " + aCarData[garageCarIdx].price, 280, 353);
    }
    // Select button
    if (owned) {
        ctx.fillStyle = isActive ? "rgba(80,220,80,0.3)" : "rgba(80,160,255,0.8)";
        ctx.beginPath();
        if(ctx.roundRect){ctx.roundRect(200, 330, 160, 36, 8);}else{ctx.rect(200, 330, 160, 36);}
        ctx.fill();
        ctx.fillStyle = "#fff";
        ctx.font = "bold 14px Arial";
        ctx.textAlign = "center";
        ctx.fillText(isActive ? "ACTIVE" : "SELECT", 280, 353);
    }
    // Back button
    ctx.fillStyle = "rgba(255,255,255,0.12)";
    ctx.beginPath();
    if(ctx.roundRect){ctx.roundRect(20, 358, 130, 34, 8);}else{ctx.rect(20, 358, 130, 34);}
    ctx.fill();
    ...
    ctx.textAlign = "center";
    ctx.fillText("< BACK", 85, 379);
    // Car dots indicator
    for (var di = 0; di < aCarData.length; di++) {
        ctx.beginPath();
        ctx.arc(W/2 - (aCarData.length-1)*10 + di*20, 285, di === garageCarIdx ? 5 : 3, 0, Math.PI*2);
        ctx.fillStyle = di === garageCarIdx ? "#f0c030" : "rgba(255,255,255,0.3)";
        ctx.fill();
    }
    // Price tag for locked cars
    if (!owned) {
        ctx.fillStyle = "rgba(240,192,48,0.9)";
        ctx.font = "bold 13px Arial";
        ctx.textAlign = "center";
        ctx.fillText("Price: " + aCarData[garageCarIdx].price + " coins", W/2, 130);
    }
}
function updateGarageEvent() {
    if (!rotatePause && "garage" == gameState) {
        var a = getDelta();
        background.updateScroll(a),
        background.renderScroll(ctx),
        renderGarageScreen(),
        renderMuteBut(),
        requestAnimFrame(updateGarageEvent)
    }
}
...
var aSpeedLines = (function() {
    var lines = [];
    for (var i = 0; i < 30; i++) {
        lines.push({ x: Math.random() * 700, y: Math.random() * 400, len: 20 + Math.random() * 60, speed: 200 + Math.random() * 300, alpha: 0.1 + Math.random() * 0.3 });
    }
    return lines;
})();
function renderSpeedLines(ctx, delta) {
    ctx.save();
    for (var i = 0; i < aSpeedLines.length; i++) {
        var l = aSpeedLines[i];
        l.x += l.speed * delta;
        if (l.x > 700 + l.len) { l.x = -l.len; l.y = Math.random() * 400; }
...
function renderGarageButton() {
    var img = assetLib.getData("carStore");
    if (!img) return;
    var bw = 80, bh = 60, bx = 310 + (110 - bw) / 2, by = 318;
    ctx.save();
    ctx.fillStyle = "rgba(0,0,0,0.55)";
    ctx.beginPath();
    ctx.roundRect ? ctx.roundRect(bx - 6, by - 4, bw + 12, bh + 8, 10) : ctx.rect(bx - 6, by - 4, bw + 12, bh + 8);
    ctx.fill();
    ctx.strokeStyle = "#f0c030";
    ctx.lineWidth = 2;
    ctx.stroke();
    ctx.drawImage(img.img, bx, by, bw, bh);
    ctx.fillStyle = "#f0c030";
    ctx.font = "bold 11px Arial";
    ctx.textAlign = "center";
    ctx.fillText("GARAGE", bx + bw / 2, by + bh + 14);
    ctx.restore();
}
...
function addFloatText(text, x, y, color) {
    aFloatTexts.push({ text: text, x: x, y: y, color: color || "#ffffff", life: 1.2, maxLife: 1.2 });
}
function renderFloatTexts(ctx, delta) {
    for (var i = aFloatTexts.length - 1; i >= 0; i--) {
        var ft = aFloatTexts[i];
        ft.life -= delta;
        if (ft.life <= 0) { aFloatTexts.splice(i, 1); continue; }
        var alpha = ft.life / ft.maxLife;
        ctx.save();
        ctx.globalAlpha = alpha;
        ctx.font = "bold 16px Arial";
        ctx.textAlign = "center";
        var ty = ft.y - (1 - alpha) * 45;
        // City theme: neon glow effect
        if (levelTheme === "city") {
...
function renderHudCoins(ctx) {
    ctx.save();
    ctx.fillStyle = "#f0c030";
    ctx.beginPath();
    ctx.arc(22, canvas.height - 22, 10, 0, Math.PI * 2);
    ctx.fill();
    ...
    ctx.lineWidth = 3;
    ctx.strokeText(coinsThisRace, 36, canvas.height - 15);
    ctx.fillText(coinsThisRace, 36, canvas.height - 15);
    ctx.restore();
}
function renderCrashFlash(ctx, delta) {
    if (crashFlashTimer > 0) {
        var alpha = crashFlashTimer / 0.3 * 0.35;
        ctx.save();
...
function loadAssets() {
    ...
    {
        id: "userCar0",
        file: "images/user_cars/userCar_01_122x82.png"
    },
    {
        id: "userCar1",
        file: "images/user_cars/userCar_02_122x82.png"
    },
    {
        id: "userCar2",
        file: "images/user_cars/userCar_03_122x82.png"
    },
    {
        id: "userCar3",
        file: "images/user_cars/userCar_04_122x82.png"
    },
    {
        id: "userCar4",
        file: "images/user_cars/userCar_05_122x82.png"
    },
    {
        id: "carStore",
        file: "images/car_store.png"
    },
    {
        id: "enemyCar",
...
function(a) {
    var b = function() {
        ...
        return a.prototype.render = function(a, b) {
            this.inc += 5 * b,
            a.drawImage(this.oSplashScreenImgData.img, 0, 0 - this.posY, this.canvasWidth, this.canvasHeight)
        },
        a
    } ();
    a.Splash = b
    ...
    var b = function() {
        ...
        a.prototype.render = function(a, b) {
            ...
            case "levelComplete":
                ...
                // Coin breakdown overlay
                var py = this.posY;
                a.save();
                a.font = "bold 13px Arial";
                a.textAlign = "left";
                var rows = [
                    { label: "Road Coins x" + Math.round((this.oScoreData.coinsThisRace - this.oScoreData.overtakeCoins) / 5), val: "+" + (this.oScoreData.coinsThisRace - this.oScoreData.overtakeCoins), color: "#f0c030" },
                    { label: "Overtakes", val: "+" + this.oScoreData.overtakeCoins, color: "#30d0f0" },
                    { label: "Position Bonus", val: "+" + this.oScoreData.posBonus, color: "#80ff80" }
                ];
                rows.forEach(function(row, i) {
                    a.fillStyle = "rgba(0,0,0,0.5)";
                    a.fillRect(200, 165 + i * 22 + py, 280, 18);
                    a.fillStyle = "rgba(255,255,255,0.7)";
                    a.fillText(row.label, 205, 179 + i * 22 + py);
                    a.fillStyle = row.color;
                    a.textAlign = "right";
                    a.fillText(row.val, 475, 179 + i * 22 + py);
                    a.textAlign = "left";
                });
                a.strokeStyle = "rgba(240,192,48,0.5)";
                a.lineWidth = 1;
                a.beginPath(); a.moveTo(200, 232 + py); a.lineTo(480, 232 + py); a.stroke();
                a.fillStyle = "#f0c030";
                a.font = "bold 14px Arial";
                a.fillText("Coins Earned:", 205, 248 + py);
                a.textAlign = "right";
                a.fillText("+" + this.oScoreData.winnings, 475, 248 + py);
                a.fillStyle = "rgba(255,255,255,0.6)";
                a.font = "12px Arial";
                a.textAlign = "left";
                a.fillText("Total Coins: " + this.oScoreData.totalCoins, 205, 266 + py);
                a.restore();
                ...
                g = 0; g < f.toString().length; g++) {
                    ...
                    a.drawImage(this.oNumbersImgData.img, d, e, this.oNumbersImgData.oData.spriteWidth, this.oNumbersImgData.oData.spriteHeight, 355 + g * this.numberSpace, 120 + this.posY, this.oNumbersImgData.oData.spriteWidth, this.oNumbersImgData.oData.spriteHeight)
                }
...
function(a) {
    var b = function() {
        ...
        return a.prototype.render = function(a) {
            // Theme accent overlay on HUD
            var themeAccent = { forest: "rgba(80,200,80,0.18)", city: "rgba(0,220,255,0.18)", desert: "rgba(240,160,40,0.18)" };
            var accent = themeAccent[levelTheme] || "rgba(255,255,255,0.1)";
            a.fillStyle = accent;
            a.fillRect(0, 0, 140, 100);
...
function(a) {
    var b = function() {
        function b(a, b, c, d, e, f, g, h, i) {
            ...
            var coinCount = Math.floor(Math.random() * 11) + 10;
            for (var j = 0; j < coinCount; j++) {
                var side = [0.25, 0.5, 0.75][Math.floor(3 * Math.random())];
                this.aCoins.push({ rowNum: Math.floor(this.segNum * 0.1 + (this.segNum * 0.85 / coinCount) * j + 5 * Math.random()), sideMultiplier: side, collected: false });
            }
        }
        ...
        b.prototype.render = function(a) {
            ...
            this.aEnemyCars[d].rowNum >= this.segNum ? (this.aEnemyCars.splice(d, 1), 4 >= startTimer && (racePos--, playSound("overtake"), coinsThisRace += 2, overtakeCoins += 2, addFloatText("OVERTAKE +2", canvas.width / 2, 180, "#30d0f0"))) : this.aEnemyCars[d].rowNum < 0 && (this.aEnemyCars.splice(d, 1), carReleasedNum++);
            ...
            // Render coins
            for (var d = this.aCoins.length - 1; d >= 0; d--) {
                if (this.aCoins[d].collected) { this.aCoins.splice(d, 1); continue; }
                var coin = this.aCoins[d];
                var rowIdx = Math.floor(coin.rowNum);
                if (rowIdx < 0 || rowIdx >= this.segNum) { coin.rowNum += .6 * this.speed * this.delta; continue; }
                var cx = this.aRowData[rowIdx].x + this.aRowData[rowIdx].scale * coin.sideMultiplier;
                var cy = this.aRowData[rowIdx].y;
                var cs = ((this.aRowData[rowIdx].scale - 30) * this.roadScaleMultiplier + 30) / 780;
                var cr = Math.max(4, 14 * cs);
                a.save();
                a.beginPath();
                a.arc(cx, cy, cr, 0, Math.PI * 2);
                a.fillStyle = "#f0c030";
                a.fill();
                a.strokeStyle = "#c8920a";
                a.lineWidth = Math.max(1, 2 * cs);
                a.stroke();
                if (cr > 5) {
                    a.fillStyle = "#c8920a";
                    a.font = "bold " + Math.round(cr * 1.2) + "px Arial";
                    a.textAlign = "center";
                    a.textBaseline = "middle";
                    a.fillText("$", cx, cy);
                }
                a.restore();
                coin.rowNum += .6 * this.speed * this.delta;
                // Collision detection — when coin reaches player zone
                if (cy > 330 + 20 * this.hillAmount && cx > this.canvasWidth / 2 - 55 && cx < this.canvasWidth / 2 + 55 && 4 >= startTimer) {
                    coin.collected = true;
                    this.callback("hitCoin");
                } else if (coin.rowNum >= this.segNum) {
                    this.aCoins.splice(d, 1);
                }
            }
        },
        b
...
function(a) {
    var b = function() {
        ...
        return a.prototype.clearData = function() {
            ...
            this.aLevelStore.push(0); // [31] winnings
            this.aLevelStore.push(0); // [32] current car id
            this.aLevelStore.push(1); // [33] owned cars bitmask (bit0=car0 default unlocked)
        },
...
leftSteerSimple = 0,
hillAmount, saveDataHandler = new Utils.SaveDataHandler("sprintclubnitro2", 9),
...
winnings = 0,
aCarData = [
    { id: 0, name: "Starter",  price: 0,    file: "images/user_cars/userCar_01_122x82.png", maxSpeed: 475, accRate: 4.0,  turnRate: 1.8  },
    { id: 1, name: "Muscle",   price: 1500,  file: "images/user_cars/userCar_02_122x82.png", maxSpeed: 520, accRate: 5.0,  turnRate: 1.4  },
    { id: 2, name: "Drifter",  price: 2500,  file: "images/user_cars/userCar_03_122x82.png", maxSpeed: 460, accRate: 3.2,  turnRate: 2.6  },
    { id: 3, name: "Street",   price: 3500,  file: "images/user_cars/userCar_04_122x82.png", maxSpeed: 490, accRate: 3.6,  turnRate: 2.1  },
    { id: 4, name: "Phantom",  price: 6000,  file: "images/user_cars/userCar_05_122x82.png", maxSpeed: 550, accRate: 3.0,  turnRate: 2.3  }
],
currentCarId = 0,
garageCarIdx = 0,
coinsThisRace = 0,
